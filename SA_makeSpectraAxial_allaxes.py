...
for i in locals:
    for ax in 'xyz':
            startSpec = time.time()
            if dataLoc == "comet":
                rFile = "%s/lin_rays/ray__%s_%d.h5"%(workdir, ax, i)
                sFile = "%s/lin_spectra/spectra_%s_%d.h5"%(workdir, ax, i)
            else:
                rFile = '%s/rayData/%s/RD%04d/lin_rays/ray__%s_%d.h5'\
                        %(workdir,simname, d,ax,i)
                sFile ='%s/rayData/%s/RD%04d/lin_spectra/spectrum_%s_%d.h5'\
                        %(workdir, simname, d, ax, i)
