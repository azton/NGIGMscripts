...
for i in locals:
    for ax in 'xyz':
        startTime = time.time()
        if dataLoc == "comet":
            rFile = "%s/lin_rays/ray_%s_%d.h5"%(dataRepo, ax, i)
        else:
            rFile = '%s/rayData/%s/RD%04d/lin_rays/ray_%s_%d.h5'\
                    %(dataRepo,simname, d, ax, i)
        row = (i // dim)
        col = (i % dim)
        xstart = (row+0.5) * dx
        ystart = (col+0.5) * dx
        if ax == 'x':
            start = np.array([0.0, xstart, ystart])
            end = start + np.array([1.0,5*dx,5*dx])
        if ax == 'y':
            start = np.array([xstart, 0.0, ystart])
            end = start + np.array([5*dx, 1.0, 5*dx])
        if ax == 'z':
